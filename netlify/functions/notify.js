  ...
  try {
    ...
    if (data.txId) {
      try {
        ...
        paymentStatus = result.verified ? "تم التحقق تلقائيًا" : "فشل التحقق — يحتاج مراجعة الإدارة";
      } catch (error) {
        console.error("Cham Cash verification in notification failed:", error.message);
        paymentStatus = "تعذر التحقق تلقائيًا — يحتاج مراجعة الإدارة";
      }
    }
    ...
    await sendEmail({
      to: ADMIN_EMAIL,
      subject: `تسجيل جديد في ورشة من الرسم إلى الكوست – ${fullName}`,
      html: `
