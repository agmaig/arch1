...
exports.handler = async (event) => {
  ...
  try {
    ...
    return {
      ...
      body: JSON.stringify({ verified: result.verified, reason: result.reason }),
    };
  } catch (error) {
    ...
    return {
      statusCode: 503,
      headers: { "Content-Type": "application/json", "Cache-Control": "no-store" },
      body: JSON.stringify({ verified: false, error: "تعذر الاتصال بخدمة التحقق حاليًا. حاول مرة أخرى." }),
    };
  }
};
